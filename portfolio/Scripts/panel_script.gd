...
func get_global_dimensions():
	...
	global_bottom_left = map_to_local(get_rect.position + Vector2i(0, get_rect.size.y))
	global_bottom_right = map_to_local(get_rect.position + get_rect.size)
	global_top_left = map_to_local(get_rect.position)
	global_top_right = map_to_local(get_rect.position + Vector2i(get_rect.size.x, 0))
...
func _get_bottom_center():
	...
	var center_x = (global_bottom_left.x + global_bottom_right.x)/2 - 16
